...
import { Format, FormatsWithoutOKLCH } from '#/models/palette.types'
...
export const ExportAllPalettesDialog = () => {
  const { getAllObjectScales, getAllVarScales } = usePalette()
  ...
  const textObjectFormat = getAllObjectScales(objectFormat)
  const textVarFormat = getAllVarScales(varFormat)
  ...
  return (
    <Dialog>
      <DialogTrigger asChild>
        <Button
          className='rounded-none rounded-r-full max-sm:px-2 max-md:px-2.5 h-11 '
          size='lg'>
          <FileDown className='' />
